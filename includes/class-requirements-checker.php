...
if ( ! class_exists( 'Requirements_Checker' ) ) {
	...
	class Requirements_Checker {
		 ...
		 *
		 * @param string $keto_recipes Name of the plugin.
		 * @param string $plugin_slug Slug of the plugin.
		 * @return boolean
		 * @since 1.0.0
		 */
		private function is_plugin_active( $keto_recipes, $plugin_slug ) {
			...
			$this->add_error_notice(
				$keto_recipes . ' is a required plugin.',
				$keto_recipes . ' needs to be installed & activated.'
			);
		 ...
		 *
		 * @param string $keto_recipes Keto Recipes.
		 * @param string $plugin_slug Plugin Slug.
		 * @param string $min_plugin_version Minimum version required of the plugin.
		 * @return boolean
		 * @since 1.0.0
		 */
		private function is_required_plugin_version_active( $keto_recipes, $plugin_slug, $min_plugin_version ) {
			...
			$this->add_error_notice(
				"{$keto_recipes} {$min_plugin_version}+ is required.",
				"{$keto_recipes} {$installed_plugin_version} is installed."
			);
		...
		private function are_required_plugins_dependency_met() {
			...
			$installed_plugins = array_filter(
				...
				function( $required_plugin_data, $required_keto_recipes ) {
					return $this->is_plugin_active( $required_keto_recipes, $required_plugin_data['plugin_slug'] );
				},
			...
			$plugins_installed_with_required_version = array_filter(
				$installed_plugins,
				function( $required_plugin_data, $required_keto_recipes ) {
					return $this->is_required_plugin_version_active( $required_keto_recipes, $required_plugin_data['plugin_slug'], $required_plugin_data['min_plugin_version'] );
				},
				ARRAY_FILTER_USE_BOTH
			);
